...
from ._test_utilities import read_env, requests_saving_es_client, create_es_client
...
@pytest.mark.asyncio
async def test_example_instantiate(es_client: AsyncElasticsearch, index_name: str) -> None:
    ...
    documents = await retriever.aget_relevant_documents("Jupiter")
    assert len(documents) > 0
    assert all(isinstance(doc, Document) for doc in documents)


@pytest.mark.asyncio
async def test_example_instantiate_with_api_key(index_name: str) -> None:
...
@pytest.mark.asyncio
async def test_example_instantiate_with_username_password(index_name: str) -> None:
...
@pytest.mark.asyncio
async def test_example_instantiate_from_cloud(index_name: str) -> None:
...
@pytest.mark.asyncio
async def test_example_instantiate_from_existing_connection(
    es_client: AsyncElasticsearch, index_name: str
) -> None:
    ...
    await index_test_data(es_client, index_name, "text")
    documents = await retriever.aget_relevant_documents("Mars")
    assert len(documents) > 0


@pytest.mark.asyncio
async def test_example_retrieve_documents(
    es_client: AsyncElasticsearch, index_name: str
) -> None:
...
@pytest.mark.asyncio
async def test_example_custom_document_mapper(
    es_client: AsyncElasticsearch, index_name: str
) -> None:
    ...
    documents = await retriever.aget_relevant_documents("test")
    assert len(documents) > 0
    assert all(isinstance(doc, Document) for doc in documents)
...
@pytest.mark.asyncio
async def test_example_multiple_indices(
    es_client: AsyncElasticsearch, index_name: str
) -> None:
...
@pytest.mark.asyncio
async def test_retrieve_neptune_moons(
    es_client: AsyncElasticsearch, index_name: str
) -> None:
    ...
    documents = await retriever.ainvoke("Neptune")
...
@pytest.mark.asyncio
async def test_example_langchain_retriever_chain(
    es_client: AsyncElasticsearch, index_name: str
) -> None:
    ...
    result = await chain.ainvoke("What are the moons of Neptune?")
