...
class MemorizedValue(WritableManagedValue[Value, Update]):
    @staticmethod
    def on(scopes: List[str]) -> ConfiguredManagedValue:
    ...
    def __init__(
        self, loop: LoopProtocol, *, typ: Type[Any], scopes: List[str], key: str
    ) -> None:
        super().__init__(loop)
        ...
        try:
            if self.loop.store:
                self.ns = ("memories",) + tuple(
                    self.loop.config[CONF][scope] for scope in scopes
                )
        except KeyError as e:
            raise ValueError(f"Scope {e.args[0]} for shared state key not in config.configurable")
